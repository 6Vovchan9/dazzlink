...
import { OUR_TEAM_LIST_EN, OUR_TEAM_LIST_RU, OUR_TEAM_LIST_UZ } from '@app/shared/constants/ourTeam.constants';
...
export class TeamPageComponent extends AbsractExample implements AfterViewInit {
  ...
  ourPersonal: Array<IAboutPersonalData> = OUR_TEAM_LIST_RU;
  showNavModal = false;
  chosenPersonData: IAboutPersonalData;
  ...
  constructor(
    ...
    this.#pagesService.currentLanguage
      ...
      .subscribe({
        next: (val) => {
          this.#setPersonalDesc(val.toLowerCase());
        },
      });
  }

  #setPersonalDesc(curLang: string): void {
    if (curLang === 'en') {
      this.ourPersonal = OUR_TEAM_LIST_EN;
    } else if (curLang === 'uz') {
      this.ourPersonal = OUR_TEAM_LIST_UZ;
    } else {
      this.ourPersonal = OUR_TEAM_LIST_RU;
    }
  }

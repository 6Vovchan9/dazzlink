export abstract class ThumbHash { // от абстрактного класса мы не можем создать instance
    ...
    rgbaToThumbHash(w, h, rgba) {
        ...
        let encodeChannel = (channel, nx, ny): any => {
    ...
    public thumbHashToDataURL(hash) {
    ...
    public async getBinaryThumbHash(href: string): Promise<Uint8Array> {
        ...
        const binaryThumbHash = this.rgbaToThumbHash(pixels.width, pixels.height, pixels.data);
